...
Add ItemWeight decimal(18,2) not null default 0
Go
...
SELECT 0 as DetailID,
    0 VoucherID,     
    (SELECT DATEADD(DAY, -1, MIN(ArrivalDate)) FROM tblSale) AS VoucherDate,        
    '-1' AS VoucherType,                             
    ID AS AccountID,                                  
...
SELECT vd.EntryID as DetailID,
    ...
    CAST('' AS VARCHAR(200)) AS ItemDescription,'' as BillNo
FROM Vouchers V
