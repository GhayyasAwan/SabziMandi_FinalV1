IF OBJECT_ID('vw_JamaRokar','V') is not NULL
BEGIN
DROP VIEW dbo.vw_JamaRokar;
END
GO
CREATE VIEW dbo.vw_JamaRokar
AS
  ...
  -- 1. Bank Credits from vw_BankEntries
   ...
   CreditAmount <> 0
      ...
      FROM vwBankAccounts) and VoucherType<>2 AND AccountID Not IN (Select AccountID from DetailAccounts da WHERE MasterID=2)

  UNION ALL

    -- 2. VoucherType = 1, Non-Bank Credit Entries from vwTrx
    Select Top 100 PERCENT
      Vno as EntryID, AccountTitle, Narration, Amount, EntryDate, Vno, MasterID, SortOrder
    From (
SELECT
        DetailID,
        AccountTitle,
        Narration,
        CreditAmount AS Amount,
        VoucherDate  AS EntryDate, Vno, MasterID,
        Case MasterID When 4 Then 666 else 555  End as SortOrder
      FROM vwTrx
      WHERE VoucherType = 1
        AND CreditAmount <> 0
        AND AccountID NOT IN (SELECT ID
        FROM vwBankAccounts UNION ALL SELECT da.ID FROM DetailAccounts da Where MasterID=2)) a
    ORDER BY sortorder,DetailID
...
SELECT
      ...
      WHERE CreditAmount <> 0
        AND AccountID IN (SELECT ID
        FROM DetailAccounts
        WHERE MasterID = 2) and VoucherType<>2
  ...
  UNION ALL
    ...
    WHERE VoucherType = 4
      AND CreditAmount <> 0
      AND MasterAccount IN (SELECT AccountTitle
      FROM MasterAccounts
      WHERE ID = 4)
