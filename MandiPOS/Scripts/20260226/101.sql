IF oBJECT_ID('dbo.GetDetailAccount') IS NOT NULL
bEGIN
DROP PROCEDURE dbo.GetDetailAccount
eND
GO
...
and (@IncludeInActive =0 or (ISNULL(IsActive,1)=1 AND acc.MasterID<>12))
GO
GO
IF OBJECT_ID('vw_BanamRokar') IS NOT NULL
BEGIN
...
SELECT 
    ...
    vd.Amount AS Amount,
    vm.VoucherDate AS EntryDate, VoucherNo as Vno, acc2.MasterID,vd.EntryID as EntryNo
FROM Vouchers vm
LEFT JOIN VoucherDetails vd ON vm.VoucherID = vd.VoucherID
LEFT JOIN DetailAccounts acc ON vd.PartyID = acc.ID
LEFT JOIN DetailAccounts acc2 ON vd.CashAccountID = acc2.ID
...
SELECT 
    ...
    sale.ArrivalDate AS EntryDate, 0 as Vno, 99 as MasterID,0 as EntryNo
FROM tblSale sale
LEFT JOIN JVEntries jv ON sale.VoucherID = jv.VoucherID
LEFT JOIN DetailAccounts acc ON jv.AccountID = acc.ID
LEFT JOIN tblCity city ON acc.CityID = city.ID
...
SELECT 
    ...
    main.ArrivalDate AS EntryDate,0 as Vno, 99 as MasterID,0 as EntryNo
FROM tblSale main
...
LEFT JOIN tblCity city ON acc.CityID = city.ID
WHERE acc.AccountTitle NOT LIKE N'نقد سیل')AS X ORDER BY X.EntryID
Union ALl
Select Vno, acc.AccountTitle,ItemDescription,DebitAmount,VoucherDate,Vno,999 as SortOrder,0 as EntryNo
