...
SELECT 
    ...
    Narration,
    CreditAmount AS Amount,
    VoucherDate  AS EntryDate, Vno,MasterID, Case MasterID When 4 Then 666 WHen 6 Then 555 When 7 Then 444 Else 445 End as SortOrder
FROM vwTrx
WHERE VoucherType = 1
  AND CreditAmount <> 0
...
WHERE CreditAmount <> 0
  AND AccountID IN (SELECT ID FROM DetailAccounts WHERE MasterID = 2)  and  VoucherType<>2
