...
namespace MandiPOS.Reports
{
    public partial class saleBill : DevExpress.XtraReports.UI.XtraReport
    {
        public saleBill(string id, int IdType = 0)
        {
            ...
            _commission.Text = commission.toRound().ToString("N0");
            _mazdoori.Text = mazdoori.toRound().ToString("N0");
            _munshiana.Text = munshiana.toRound().ToString("N0");
            _kraya.Text = karaya.toRound().ToString("N0");
            //_store.Text = store.ToString("N0");
            _total.Text = total.toRound().ToString("N0");
            _expenses.Text = total.toRound().ToString("N0");
            _netSale.Text = (Grossale.toRound()    - total.toRound()).ToString("N0");
            decimal tobePaid = (Grossale.toRound() - total.toRound() - karaya.toRound()) - sale.PaidAmount.toRound();
            rowPaid.Visible = true;
            lblNetPaidValue.Text = sale.PaidAmount.toRound().ToString("0,0.##");
                rowPaid.Visible = true;
                lblRemAmountValue.Text = Math.Abs(tobePaid.toRound()).ToString("0,0.##");
