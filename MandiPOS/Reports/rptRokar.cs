...
";
            ...
            decimal prevJama = jama.Compute("Sum(Amount)", string.Empty) is DBNull ? 0 : Convert.ToDecimal(jama.Compute("Sum(Amount)", string.Empty));
            decimal prevBanam = banam.Compute("Sum(Amount)", string.Empty) is DBNull ? 0 : Convert.ToDecimal(banam.Compute("Sum(Amount)", string.Empty));

            var dtOldCash = General.FetchRecords(@"Select Sum(opdebit - opCredit) as Amount From DetailAccounts Where ID=(Select Top 1 ID From DetailAccounts where MasterID=10)", null);
            decimal opCash = (dtOldCash?.Rows.Count > 0) ? (dtOldCash.Rows[0]["Amount"] is DBNull ? 0 : Convert.ToDecimal(dtOldCash.Rows[0]["Amount"])) : 0;

            // ✅ FIXED: Subtract Jama (credit), Add Banam (debit)
            openingCash = opCash - prevJama + prevBanam;
...
Select 0 as EntryID, acc.AccountTitle+' '+city.CityName as 'AccountTitle',vd.Narration+' =>'+acc2.AccountTitle as 'Narration',vd.Amount as 'Amount' from Vouchers vm
Left join VoucherDetails vd on vm.VoucherID=vd.VoucherID
left join DetailAccounts acc on vd.PartyID=acc.ID
left join tblCity city on acc.CityID=city.ID
left join DetailAccounts acc2 on vd.CashAccountID=acc2.id
...
and vd.CashAccountID in (Select ID from vwBankAccounts)
union all
Select 0 as EntryID, acc.AccountTitle+' '+city.CityName as 'AccountTitle',vd.Narration,vd.Amount as 'Amount' from Vouchers vm
Left join VoucherDetails vd on vm.VoucherID=vd.VoucherID
left join DetailAccounts acc on vd.PartyID=acc.ID
left join DetailAccounts acc2 on vd.CashAccountID=acc2.id
left join tblCity city on acc.CityID=city.ID
...
and vd.CashAccountID Not in (Select ID from vwBankAccounts)
union all
Select 0 as EntryID, acc2.AccountTitle+' '+city.CityName as 'AccountTitle',vd.Narration+' =>'+acc.AccountTitle as 'Narration',vd.Amount as 'Amount' from Vouchers vm
Left join VoucherDetails vd on vm.VoucherID=vd.VoucherID
left join DetailAccounts acc on vd.PartyID=acc.ID
left join DetailAccounts acc2 on vd.CashAccountID=acc2.id
left join tblCity city on acc2.CityID=city.ID
...
union all
 ...
 Select ArrivalNo as EntryID,AccountTitle+' '+CityName as 'AccountTitle',Narration,DebitAmount as 'Amount' from tblSale sale left join tblSaleDetail detail on sale.id=detail.SaleID
  left join JVEntries jv on sale.VoucherID=jv.VoucherID
  left join DetailAccounts acc on jv.AccountID=acc.ID
  left join tblCity city on acc.CityID=city.ID
  Where sale.ArrivalDate=@date and DebitAmount<>0 and MasterID=4
union all
Select * from (Select Top (100) percent ArrivalNo as EntryID, acc.AccountTitle+' '+city.CityName as 'AccountTitle', ItemTitle+' '+Cast(Case when ItemUnit=0 then Cast(Floor(ItemQty) as Nvarchar) else Cast(Floor(ItemWeight) as nvarchar)  End as nvarchar)+'*'+Cast(Floor(CustomerRate) as nvarchar) as 'Narration',CustomerAmount+LagaAmount as 'amount' from tblSale main 
left join tblSaleDetail sdetail on main.id=sdetail.SaleID
left join tblItems on sdetail.ItemID=tblItems.ID
left join DetailAccounts acc on sdetail.PartyID=acc.ID
left join tblcity city on acc.CityID=city.ID
...
Select 0 as EntryID, AccountTitle,Narration,Sum(CreditAmount) as 'Amount' from vw_BankEntries Where VoucherDate=@date and CreditAmount<>0 and AccountID in (Select ID from vwBankAccounts) Group By AccountTitle,Narration
union All
Select 0 as EntryID, AccountTitle,Narration,Sum(CreditAmount) as 'Amount' from vwTrx
Where 1=1 and VoucherType=1 and CreditAmount<>0 and AccountID not in (Select ID from vwBankAccounts)
and VoucherDate=@date
Group By AccountTitle,Narration
union All
Select 0 as EntryID,AccountTitle,'' as Narration,Sum(CreditAmount) as 'Amount' from vwTrx
Where 1=1 and  CreditAmount<>0 and AccountID in (Select ID From DetailAccounts Where MasterID=2)
and VoucherDate=@date
Group by AccountTitle

union all
Select BillNo as EntryID, AccountTitle,Narration,CreditAmount as Amount from (Select * from vwTrx
Where VoucherType=4 and CreditAmount<>0
and VoucherDate=@date) tbl 
where MasterAccount in (Select AccountTitle from MasterAccounts where ID=4)
";
